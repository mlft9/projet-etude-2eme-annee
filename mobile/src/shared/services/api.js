...
export function fetchParcelles(token) {
  ...
  return Promise.resolve([
    { id: 1, name: 'Champ Nord (BlÃ©)', culture: 'BlÃ© d\'hiver', surface_ha: 15, latitude: 43.60, longitude: 1.44 },
    { id: 2, name: 'Vigne Sud', culture: 'Vigne', surface_ha: 5.5, latitude: 43.58, longitude: 1.45 },
    { id: 3, name: 'Parcelle MaÃ¯s', culture: 'MaÃ¯s doux', surface_ha: 12, latitude: 43.62, longitude: 1.42 }
  ]);
}
...
export function fetchDiagnostics(token) {
  // DEMO MODE: Hardcoded rich dataset
  return Promise.resolve([
    { id: 101, parcelle_id: 1, resultat: 'Rouille jaune', score_confiance: 85, niveau_risque: 'Ã‰levÃ©', created_at: new Date().toISOString() },
    { id: 102, parcelle_id: 2, resultat: 'Mildiou (dÃ©but)', score_confiance: 65, niveau_risque: 'Moyen', created_at: new Date(Date.now() - 86400000).toISOString() },
...
export async function fetchSoilHealth(token, parcelleId) {
  try {
    ...
    return {
      globalScore: Math.floor(Math.random() * (95 - 40) + 40),
      metrics: {
        hydration: Math.floor(Math.random() * (100 - 30) + 30),
        nutrition: Math.floor(Math.random() * (100 - 30) + 30)
      },
...
export function fetchCapteurs(token) {
  // DEMO MODE: Hardcoded rich dataset
  return Promise.resolve([
    { id: 'c1', name: 'Sonde Temp/Hum - BlÃ©', serial_number: 'SN-001', parcelle_id: 1, latest: { temp: 18, humidite: 45 } },
    { id: 'c2', name: 'Station Vigne', serial_number: 'SN-002', parcelle_id: 2, latest: { temp: 22, humidite: 60 } },
    { id: 'c3', name: 'Sonde MaÃ¯s (Irrigation)', serial_number: 'SN-003', parcelle_id: 3, latest: { temp: 25, humidite: 30 } }
  ]);
}
...
// SIMULATION: Dataset Bétail
export async function fetchBetail(token) {
  return Promise.resolve([
    { id: 'b1', type: 'Bovins (Laitiers)', race: "Prim'Holstein", count: 45, chips: 45, localisation: 'Pâturage Nord', coords: { lat: 43.6, lng: 1.4 } },
    { id: 'b2', type: 'Bovins (Viande)', race: 'Charolaise', count: 20, chips: 20, localisation: 'Étable Principale', coords: { lat: 43.61, lng: 1.41 } },
